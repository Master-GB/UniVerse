...
// Add new question under a faculty & role
  ...
  try {
    ...
    if (!faculty) {
      ...
      if (!role) {
        ...
        role.jobType = jobType; // Update job type if changed
      }
    }
...
const getQuestionsByFacultyAndRole = async (req, res) => {
  try {
    ...
    // return questions and some role metadata
    res.status(200).json({
...
// Return a compact list of faculties and their role names for frontend dropdowns
const getFaculties = async (req, res) => {
  try {
    const faculties = await InterviewQuiz.find({}, 'facultyName roles.roleName');
    // transform to a simple structure: [{ _id, facultyName, roles: [{ roleName }] }]
...
// Update quiz (specific role's questions)
